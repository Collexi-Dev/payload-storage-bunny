        ...
        try {
            ...
            if (stream && videoId) {
                if (!stream.mp4FallbackQuality) {
                    return new Response('MP4 fallback quality not configured', {
                        status: 400
                    });
                }
            ...
            const response = await ky.get(`https://${getStorageUrl(storage.region)}/${storage.zoneName}/${posix.join(prefix, filename)}`, {
                headers: {
